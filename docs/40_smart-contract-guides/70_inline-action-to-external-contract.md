...
```
The first new concept in the code above is that we are explicitly restricting calls to the one action to a **specific account** in this contract using [require_auth](https://docs.eosnetwork.com/manuals/cdt/latest/group__action/#function-require_auth) to the `addressbook` contract, as seen below.
```cpp
...
Another new concept in the code above, is [action wrapper](https://docs.eosnetwork.com/manuals/cdt/latest/structeosio_1_1action__wrapper). As shown below the first template parameter is the 'action' we are going to call and the second one should point to the action function
```text
using count_action = action_wrapper<"count"_n, &abcounter::count>;
```
...
- For the permission, [get_self()](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1contract/#function-get_self)  returns the current `addressbook` contract. The `active` permission of `addressbook` is used.
