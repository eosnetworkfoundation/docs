...
- A [permission_level](https://docs.eosnetwork.com/manuals/cdt/latest/structeosio_1_1permission__level) struct
...
```
The last entry in the previous log is an `addressbook::notify` action sent to `alice`. Use [cleos get actions](https://docs.eosnetwork.com/manuals/eos/v2.2/cleos/command-reference/get/actions) to display actions executed and relevant to alice.
```shell
