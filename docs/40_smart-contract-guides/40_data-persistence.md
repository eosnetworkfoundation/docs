...
Now that the data structure of the table has been defined with a `struct` we need to configure the table. The [eosio::multi_index](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1multi__index) constructor needs to be named and configured to use the struct we previously defined.
...
On a Antelope blockchain an account name is unique, therefore the `name` type is an ideal candidate as a **primary_key**. Behind the scenes, the [name](https://docs.eosnetwork.com/manuals/cdt/latest/structeosio_1_1name) type is an `uint64_t` integer.
...
Create a record in the table using the multi_index method [emplace](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1multi__index/#function-emplace). This method accepts two arguments, the "payer" of this record who pays the storage usage and a callback function.
...
Next, handle the modification, or update, case of the "upsert" function. Use the [modify](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1multi__index/#function-modify-12) method, passing a few arguments:
...
Instantiate the table. In `addressbook` each account has only one record. Set `iterator` with [find](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1multi__index/#function-find)
...
Finally, call the [erase](https://docs.eosnetwork.com/manuals/cdt/latest/classeosio_1_1multi__index/#function-erase-12) method, to erase the iterator. Once the row is erased, the storage space will be free up for the original payer.
...
[cdt](https://docs.eosnetwork.com/manuals/cdt/latest) includes an ABI Generator, but for it to work will require some declarations.
