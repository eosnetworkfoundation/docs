...
The `Antelope framework` provides a blockchain framework and a key feature of the `Antelope framework` is it's flexibility. Smart Contracts are part of this flexibility. Many features are implemented using smart contracts, and as smart contracts can be edited and built so you can customise blockchain behaviour. Some examples are consensus, governance and resource models. For more information see [eos-system-contracts](https://developers.eos.io/manuals/eos-system-contracts/latest/index) and the [bios boot sequence tutorial.](../../80_tutorials/10_bios-boot-sequence.md)    
