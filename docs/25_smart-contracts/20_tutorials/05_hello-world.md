...
The above action accepts a parameter called `user`, that's a [`name`](http://docs.eosnetwork.com/reference/) type. EOS comes with a number of typedefs, one of the most common typedefs you'll encounter is `name`. Using the `eosio::print` library previously included, concatenate a string and print the `user` parameter. Use the braced initialization of `name{user}` to make the `user` parameter printable.
...
[[info]]
| The ABI Generator in eosio.cdt supports several different style of attributes, see the ABI usage guide [here](./15_understanding-ABI-files.md).
