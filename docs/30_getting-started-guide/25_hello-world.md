...
* [Antelope Contract Development Toolkit](https://docs.eosnetwork.com/manuals/cdt/latest/index) - Toolchain and libraries used to build smart contracts
...
* [Application Binary Interfaces](https://docs.eosnetwork.com/manuals/cdt/latest/best-practices/abi/understanding-abi-files) (ABI) - The interface that defines how data is marshalled to and from the webassembly virtual machine
...
* Compile and deploy the smart contract to an Antelope blockchain
...
compile and deploy the smart contract to the blockchain. Use the Antelope CDT [eosio-cpp](https://docs.eosnetwork.com/manuals/cdt/latest/command-reference/eosio-cpp) command to build the `.wasm` file and a corresponding `.abi` file. 
...
Once the smart contract is successfully deployed, follow this section to [push smart contract actions](https://docs.eosnetwork.com/manuals/eos/v2.0/cleos/command-reference/push/push-action) to the blockchain and test the `hi` action.
...
## Authorization
The Antelope blockchain uses asymmetric cryptography to verify that the account pushing a transaction has signed the transaction with the matching private key. Antelope blockchains use account authority tables to check the account has the required authority to perform an action. Using authorization is the first step towards [securing your smart contract.](https://docs.eosnetwork.com/manuals/cdt/v1.7/best-practices/securing_your_contract) Follow this link [for more information about authorization checks.](https://docs.eosnetwork.com/manuals/cdt/v1.8/how-to-guides/authorization/how_to_restrict_access_to_an_action_by_user)

Add [require_auth](https://docs.eosnetwork.com/manuals/cdt/latest/group__action#function-require_auth) to the smart contract, the `require_auth` function checks authorization and ensures the name parameter matches the user executing and authorizing the action. 
