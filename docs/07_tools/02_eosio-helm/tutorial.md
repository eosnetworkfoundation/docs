---
content_title: Antelope Helm Tutorial
link_text: Antelope Helm Tutorial
---

## Overview

This tutorial demonstrates how to install and use Helm charts for local Antelope deployment using Kubernetes and Docker Desktop. The tutorial is divided into three parts: [Installation](#part-1-installation), [Basic Usage](#part-2-basic-usage), and [Antelope Helm Repositories](#part-3-eosio-helm-repos). The concepts will be illustrated within the context of a single Antelope node being deployed via Kubernetes and hosted within an Antelope docker image.
...
* Read the [Antelope Helm Charts tool](index.md) guide.
...
Perform the following steps to install Antelope Helm Charts for local deployment:

1. Clone the Antelope Help Charts github repository:
...
5. Install the Antelope Helm chart itself:
...
* You can also query the logs generated by the running Antelope node instance:
...
Note that for a full cleanup you also need to remove the state generated by the Antelope node. To that end, first list all Kubernetes persistent artefacts:
...
[[info | Note]]
| If you want to retain the Antelope node state in between launches and cleanups, then do not perform the last step since it will destroy the state stored within the Antelope persistent volume.

## Part 3: Antelope Helm Repos
...
Perform any of the following steps to manage Antelope Helm repositories:
...
This tutorial showcased how to install an Antelope Helm chart. It also explained how to use it within the context of a deployed single Antelope node. Finally, it demonstrated how to create an Antelope Helm repository for sharing Antelope Helm charts.
